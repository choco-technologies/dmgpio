...
{
    if (port >= STM32F4_GPIO_PORT_COUNT) return -1;
    if (power_on)
        STM32F4_RCC_AHB1ENR |= (1U << port);
    else
        STM32F4_RCC_AHB1ENR &= ~(1U << port);
    (void)STM32F4_RCC_AHB1ENR; /* read-back for clock stabilisation */
    return 0;
}
...
{
    /* STM32F4 does not have a separate current register */
...
{
    if (port >= STM32F4_GPIO_PORT_COUNT || pin > 15U || out_used == NULL) return -1;
