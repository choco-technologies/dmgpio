...
#define STM32F4_RCC_APB2ENR   (*(volatile uint32_t *)(STM32F4_RCC_BASE + 0x44U))
#define STM32F4_SYSCFG_APB2BIT  14U  /* SYSCFGEN in RCC_APB2ENR */

#define STM32F4_GPIO_PORT_COUNT 9U

static const uint32_t gpio_base_addresses[STM32F4_GPIO_PORT_COUNT] = {
    STM32F4_GPIOA_BASE, STM32F4_GPIOB_BASE, STM32F4_GPIOC_BASE,
    STM32F4_GPIOD_BASE, STM32F4_GPIOE_BASE, STM32F4_GPIOF_BASE,
    STM32F4_GPIOG_BASE, STM32F4_GPIOH_BASE, STM32F4_GPIOI_BASE,
};

/* Per-port pin usage bitmasks */
static uint16_t pin_used_mask[STM32F4_GPIO_PORT_COUNT];
...
/**
 * @brief Configure a 2-bit field in a 32-bit register for each set bit in pins mask.
 * Each pin occupies 2 bits at position (pin * 2).
 */
...
/**
 * @brief Read a 2-bit field for the first set bit in pins mask.
 */
...
{
    for (dmgpio_port_t i = 0; i < STM32F4_GPIO_PORT_COUNT; i++)
        pin_used_mask[i] = 0;
...
{
    if (port >= STM32F4_GPIO_PORT_COUNT || pins == 0)
    {
    ...
    STM32F4_RCC_APB2ENR |= (1U << STM32F4_SYSCFG_APB2BIT);
    (void)STM32F4_RCC_APB2ENR;
    return 0;
}
    ...
    ( dmgpio_port_t port, dmgpio_pins_mask_t pins, dmgpio_protection_t protection ))
{
    ...
    /* STM32 LCKR unlock sequence: write LCKK=1 + ~pins, LCKK=0 + ~pins, LCKK=1 + ~pins, then read */
    ...
    ( dmgpio_port_t port, dmgpio_pins_mask_t pins ))
{
    ...
    /* STM32 LCKR lock sequence: write LCKK=1 + pins, LCKK=0 + pins, LCKK=1 + pins, then read */
    volatile uint32_t old_val = gpio->LCKR;
    uint32_t base = old_val | (uint32_t)pins;
    gpio->LCKR = (1U << 16U) | base;
    gpio->LCKR = (0U << 16U) | base;
...
{
    /* STM32F4 does not have a separate current register - strength is encoded via speed */
    ...
    ( dmgpio_port_t port, dmgpio_pins_mask_t pins, dmgpio_int_trigger_t trigger ))
{
    ...
    {
        /* Disable interrupt for all pins in mask */
    ...
    {
        DMOD_LOG_ERROR("Level-triggered interrupts are not supported on STM32F4\n");
        return -1;
    }
...
{
    if (port >= STM32F4_GPIO_PORT_COUNT || pins == 0 || out_trigger == NULL)
    {
    ...
    ( dmgpio_port_t port, dmgpio_pins_mask_t pins ))
{
    ...
    pin_used_mask[port] &= ~pins;
    /* Disable any EXTI interrupts for these pins */
    EXTI_IMR  &= ~(uint32_t)pins;
    EXTI_RTSR &= ~(uint32_t)pins;
    EXTI_FTSR &= ~(uint32_t)pins;
    return 0;
...
{
    if (port >= STM32F4_GPIO_PORT_COUNT || pins == 0 || out_used == NULL)
    {
